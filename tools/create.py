import sys
import json
import requests
from gringotts.client import client

OS_AUHT_URL = 'http://localhost:35357/v3'

client = client.Client(username='admin',
                       password='rachel',
                       project_name='admin',
                       auth_url=OS_AUHT_URL)

f = open('user.log')
_emails = f.readlines()

emails = []
for email in _emails:
    emails.append(email.strip('\n'))

def get_uos_user(email):

    internal_api = lambda api: OS_AUHT_URL + '/US-INTERNAL'+ '/' + api

    query = {'query': {'email': email}}
    r = requests.post(internal_api('get_user'),
                      data=json.dumps(query),
                      headers={'Content-Type': 'application/json'})
    if r.status_code == 404:
        sys.exit('can not find the user')
    return r.json()['user']


def charge_account(project_id, value, type, come_from):
    body = dict(value=value,
                type=type,
                come_from=come_from)
    client.put('/accounts/%s' % project_id, body=body)

def create_account(user_id, project_id):
    body = dict(user_id=user_id,
                project_id=project_id,
                consumption='0',
                balance='0',
                level=3,
                currency='CNY')
    client.post('/accounts', body=body)


if __name__ == '__main__':
    for email in emails:
        print email
        user = get_uos_user(email)
        print user['id'], user['default_project_id']
        #create_account(user['id'], user['default_project_id'])
        charge_account(user['default_project_id'], 10, 'bonus', 'system')
